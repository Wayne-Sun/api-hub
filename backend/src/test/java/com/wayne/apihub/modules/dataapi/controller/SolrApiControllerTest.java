...
class SolrApiControllerTest {

    @Mock
    private DataApiConfService dataApiConfService;

    @Mock
    private SolrApiHandlerFactory solrApiHandlerFactory;

    @InjectMocks
    private SolrApiController solrApiController;
    ...
    @BeforeEach
    void setUp() {
        mockMvc = MockMvcBuilders.standaloneSetup(solrApiController).build();
    }

    @Test
    void registerApi_success_shouldReturn200() throws Exception {
        SolrApiConf apiConf = new SolrApiConf();
        apiConf.setName("test-solr-api");
        apiConf.setCollection("test_collection");
        apiConf.setFields("field1,field2");

        doNothing().when(solrApiHandlerFactory).registerApi(any(SolrApiConf.class));

        mockMvc.perform(post("/v1/api/solr/registerApi")
    ...
    void registerApi_exception_shouldReturn500() throws Exception {
        ...
        doThrow(new RuntimeException("Register failed"))
                .when(solrApiHandlerFactory).registerApi(any(SolrApiConf.class));
    ...
    void listApi_success_shouldReturn200() throws Exception {
        ...
        when(dataApiConfService.listSolrApiConfs(1, 10))
                .thenReturn(BaseResponse.ok("mock data"));

        mockMvc.perform(post("/v1/api/solr/listApi")
                        .contentType(MediaType.APPLICATION_JSON)
    ...
    void enableApi_success_shouldReturn200() throws Exception {
        doNothing().when(solrApiHandlerFactory).enableApi(anyLong());

        mockMvc.perform(get("/v1/api/solr/enableApi")
                        .param("id", "1"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.code").value(200));
    }
    ...
    void enableApi_DataApiException_shouldReturn400() throws Exception {
        doThrow(DataApiException.alreadyEnabled())
                .when(solrApiHandlerFactory).enableApi(anyLong());
    ...
    void disableApi_success_shouldReturn200() throws Exception {
        doNothing().when(solrApiHandlerFactory).disableApi(anyLong());

        mockMvc.perform(get("/v1/api/solr/disableApi")
                        .param("id", "1"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.code").value(200));
    }
    ...
    void disableApi_DataApiException_shouldReturn400() throws Exception {
        doThrow(DataApiException.alreadyDisabled())
                .when(solrApiHandlerFactory).disableApi(anyLong());
    ...
    @Test
    void disableApi_genericException_shouldReturn500() throws Exception {
        doThrow(new RuntimeException("Unexpected error"))
                .when(solrApiHandlerFactory).disableApi(anyLong());
