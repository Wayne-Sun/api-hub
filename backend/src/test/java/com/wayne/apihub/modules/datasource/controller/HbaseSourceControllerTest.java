...
class HbaseSourceControllerTest {
    ...
    @InjectMocks
    private HbaseSourceController controller;
    ...
    void insertSource_success_returns200() throws Exception {
        ...
        conf.setCoreSitePath("/path/to/core-site.xml");

        when(dataSourceConfService.insertHbaseSourceConf(any())).thenReturn(BaseResponse.ok());

        mockMvc.perform(post("/v1/source/hbase/insertSource")
    ...
    void insertSource_serviceThrowsException_returns500() throws Exception {
        ...
        when(dataSourceConfService.insertHbaseSourceConf(any())).thenThrow(new RuntimeException("DB connection failed"));
        ...
        mockMvc.perform(post("/v1/source/hbase/insertSource")
                ...
                .andExpect(jsonPath("$.data").value("DB connection failed"));
    }
    ...
    void listSource_success_returns200() throws Exception {
        ...
        when(dataSourceConfService.listHbaseSourceConfs(anyInt(), anyInt())).thenReturn(BaseResponse.ok("list data"));

        mockMvc.perform(post("/v1/source/hbase/listSource")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(pageRequest)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.code").value(200))
                .andExpect(jsonPath("$.message").value("SUCCESS"))
                .andExpect(jsonPath("$.data").value("list data"));
    }
    ...
    void initSource_success_returns200() throws Exception {
        ...
        doNothing().when(hbaseClientFactory).initClient(any());

        mockMvc.perform(post("/v1/source/hbase/initSource")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(conf)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.code").value(200))
    ...
    void enableSource_success_returns200() throws Exception {
        doNothing().when(hbaseClientFactory).enableClient(anyLong());

        mockMvc.perform(get("/v1/source/hbase/enableSource")
                        .param("id", "1"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.code").value(200))
                .andExpect(jsonPath("$.message").value("SUCCESS"));
    ...
    void enableSource_dataSourceException_returns400() throws Exception {
        doThrow(DataSourceException.alreadyEnabled()).when(hbaseClientFactory).enableClient(anyLong());
    ...
    void disableSource_success_returns200() throws Exception {
        doNothing().when(hbaseClientFactory).disableClient(anyLong());

        mockMvc.perform(get("/v1/source/hbase/disableSource")
                        .param("id", "1"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.code").value(200))
                .andExpect(jsonPath("$.message").value("SUCCESS"));
    ...
    void disableSource_dataSourceException_returns400() throws Exception {
        doThrow(DataSourceException.alreadyDisabled()).when(hbaseClientFactory).disableClient(anyLong());
