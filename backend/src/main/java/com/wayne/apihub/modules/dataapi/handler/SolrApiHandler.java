...
public class SolrApiHandler implements CommonApiHandler {
    ...
    public SolrApiHandler(SolrApiConf solrApiInfo) {
        ...
        String[] conditionArray = solrApiInfo.getConditions().split(Constants.COMMA);
        String[] fieldArray = solrApiInfo.getFields().split(Constants.COMMA);
        conditions.addAll(Arrays.asList(conditionArray));
        fields.addAll(Arrays.asList(fieldArray));
        String[] orderArray = solrApiInfo.getOrders().split(Constants.COMMA);
        for (String order : orderArray) {
            String[] splitOrder = order.split(Constants.COLON);
    ...
    public SolrQuery generateQuery(List<String> params, Integer pageNum, Integer pageSize) {
        ...
        for (int i = 0, size = params.size(); i < size; i++) {
            queryBuilder.append(conditions.get(i)).append(Constants.COLON).append(params.get(i));
        }
