...
public class SolrClient implements CommonClient {
    ...
    public SolrClient(SolrSourceConf info) {
        solrClient = new CloudSolrClient.Builder(Arrays.asList(info.getZkHosts().split(Constants.COMMA).clone()), Optional.ofNullable(info.getZkChroot()))
                .withConnectionTimeout(Constants.SOLR_CONN_TIMEOUT)
                .build();
        solrClient.connect();
    }
